...
func (c *Client) Launch(ctx context.Context, params acp.LaunchParams) error {
	...
	if err != nil {
		_ = c.Close(context.Background())
		return fmt.Errorf("claudeacp: initialize negotiation: %w", err)
	}
	...
	if params.SessionIDPreset != "" && initialize.LoadSession {
		if err := c.loadSession(ctx, params); err != nil {
			_ = c.Close(context.Background())
			return fmt.Errorf("claudeacp: session/load: %w", err)
		}
		c.mu.Lock()
...
func (c *Client) Prompt(ctx context.Context, prompt string) error {
	...
	if c.currentTurnID != "" {
		c.turnMu.Unlock()
		return errors.New("claudeacp: a turn is already in flight")
	}
	c.mu.Lock()
	if c.closed {
		c.mu.Unlock()
		c.turnMu.Unlock()
		return errors.New("claudeacp: client is closed")
	}
