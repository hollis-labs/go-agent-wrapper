...
func (c *Client) Launch(ctx context.Context, params acp.LaunchParams) error {
	...
	if params.SessionIDPreset != "" {
		if err := c.loadSession(ctx, params); err == nil {
			c.mu.Lock()
			// pi-acp's session/load result does NOT carry a sessionId
			// field (unlike session/new's) — the caller is expected to
			// keep using the id it requested resume with. See the
			// package doc's "Real wire behavior" section.
			c.sessionID = params.SessionIDPreset
			c.mu.Unlock()
		} else {
			// Best-effort resume only, matching acp.LaunchParams's own
			// documented "implementations that don't support resume
			// ignore it silently" contract — a stale/unknown session id
			// falls back to a fresh session rather than failing Launch.
			if err := c.newSession(ctx, params); err != nil {
				_ = c.Close(context.Background())
				return fmt.Errorf("piacp: session/new (after session/load fallback): %w", err)
			}
		}
...
func (c *Client) waitProcess() {
	...
	c.failPending(errors.New("piacp: process exited before response"))
...
func (c *Client) drainStderr(r io.Reader) {
	...
	for scanner.Scan() {
		...
		// noise, not protocol frames. Intentionally dropped rather than
		// misclassified as protocol activity — same convention
		// opencodeacp/copilotacp already use for their own stderr.
		_ = scanner.Text()
	}
}
